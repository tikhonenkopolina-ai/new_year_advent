...
Используем встроенный сервер webhook из python-telegram-bot (run_webhook),
чтобы упростить деплой на Render.
"""
...
def get_today_index() -> int:
    """
    Вернуть индекс дня адвента (0..len-1) для текущей даты.

    Логика без ограничений:
    - берём разницу между сегодняшней датой и ADVENT_START;
    - берём её по модулю длины списка ADVENT_DAYS;
    - таким образом, дни идут по кругу и всегда что-то выдаётся.
    """
...
def build_main_keyboard() -> InlineKeyboardMarkup:
    """Кнопка «Распаковать подарок на сегодня»."""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        "Привет, любимый! 💌\n\n"
        "Этот бот будет каждый день подбрасывать тебе маленький подарок: "
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        ...
        "Сейчас нет привязки к датам — всё можно тестировать в любой день."
    )
    if update.message:
        await update.message.reply_text(text, reply_markup=keyboard)
    elif update.callback_query:
...
async def handle_unpack_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Обработка нажатия на кнопку «🎁 Распаковать подарок на сегодня».
...
def main() -> None:
    ...
    # Запускаем встроенный веб-сервер и регистрируем вебхук
